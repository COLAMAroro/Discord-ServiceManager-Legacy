...
namespace Discord_ServiceManager
{
    static class MessageProcess
    {
        private static async void status(SocketMessage message, Service service)
        {
            ...
            _ = await message.Channel.SendMessageAsync(string.Format("{0} is  {1}", service.name, status_str));
            return;
        }
        ...
        private static async void stop (SocketMessage message, Service service)
        {
            ...
            {
                _ = await message.Channel.SendMessageAsync(string.Format("{0} is already stopped", service.name));
                return;
            }
            sc.Stop();
            ...
            _ = await message.Channel.SendMessageAsync(string.Format("{0} sucessfully stopped", service.name));
        }
        ...
        private static async void start (SocketMessage message, Service service)
        {
            ...
            {
                _ = await message.Channel.SendMessageAsync(string.Format("{0} is already running", service.name));
                return;
            }
            sc.Start();
            int counter = 0;
            ...
            if (counter == 10 && sc.Status != ServiceControllerStatus.Running)
                _ = await message.Channel.SendMessageAsync(string.Format("{0} did not start sucessfully. Contact server hoster", service.name));
            else
                _ = await message.Channel.SendMessageAsync(string.Format("{0} sucessfully started", service.name));
        }
